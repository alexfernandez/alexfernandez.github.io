...
and then the proportion of heat we can send back to space `p(heat)`.
Finally we can estimate how much can we affect the temperature of the planet
`ΔT(mirror)`,
where the capital delta `Δ` represents the difference.

### 🧮 How Many Mirrors

Let us begin by estimating the area covered by buildings on the planet.
How much space do we as a species actually occupy?
...
we will add twice the space, getting to 30 m² per person.
...
Converting to square kilometers with `1 km² = 1000000 m²`:
...
What really interests us is the proportion of the total surface of the planet
that we can cover in mirrors, `p(mirror)`.
...
Or put another way,
how much incoming heat can we reflect back to space, `p(heat)`?
...
Therefore if we can radiate a part of this heat
we will be able to lower the temperature accordingly.

Let's work out the details.
To ease our task we will work in
[kelvins](https://en.wikipedia.org/wiki/Kelvin),
which are equal in magnitude to degrees Celsius but start at absolute zero (-273 C).
...
Let us check our estimate for `p(buildings)` with independent data.
...
I have to admit that I tweaked it a bit after reading the above pages,
but it's nice to see that the numbers make sense.
...
[around 0.8 C per decade (🇪🇸)](https://ciencia.nasa.gov/el-mar-de-invernaderos-de-almeria),
because they reflect so much heat back to space.
...
which would give us a temperature difference of:
...
Even if the solar part is too complex,
perhaps we can just use large aluminium foil panels above crops
to both improve yields and cool the land around it.
There are still the chemical consequences of having so much carbon dioxide in the atmosphere,
but at least we might go back to tolerable temperatures.
...
In the process we built a Fermi estimation for how the Earth cools due to reflected sunlight,
that has allowed us to explore other possible extensions.
It is in fact so powerful that we will use it for our next installment.

Even some local cooling can be a welcome effect;
and together with some other measures it can help relieve our poor overheated planet.
...
* Part 3: 🪩 Climate Change: Mirror Roofss,
